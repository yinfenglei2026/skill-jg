...
Release registration accepts only `oci://...@sha256:<64 hex>` references and derives the stored digest from that reference. The caller must be an authenticated `OPERATOR`, normally a CI service identity. Outside the test and local profiles, startup fails unless the Harbor/Cosign settings are complete: `HARBOR_REGISTRY`, `HARBOR_USERNAME`, `HARBOR_PASSWORD`, `COSIGN_EXECUTABLE`, `COSIGN_PUBLIC_KEY`, and a nonempty `SLSA_ALLOWED_BUILDER_IDS`; `HARBOR_CA_CERT` is an optional absolute path to a PEM CA that is added to the JVM trust store. The Harbor account must be a read-only robot scoped to the required repositories. The production verifier authenticates the OCI manifest request, requires the returned `Docker-Content-Digest` to equal the requested digest, verifies a fixed-key Cosign signature with the pinned `v3.0.6` executable, and requires exactly one SLSA v1 provenance statement bound to the digest, source repository, source revision, and an allowed builder. `REGISTRY_DIGEST`, `COSIGN_SIGNATURE`, and `SLSA_PROVENANCE` evidence records are persisted atomically only after all checks pass. Failures remain fail-closed and are classified as registry policy/auth/access/availability/protocol, digest mismatch, Cosign availability/timeout/output, signature invalid, or provenance missing/invalid/policy mismatch. Registry credentials are supplied through a temporary Docker auth workspace and insecure Cosign registry flags are forbidden. Local/test profiles retain explicit non-production evidence and do not claim external trust.
