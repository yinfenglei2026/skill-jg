...
- Live Harbor/Cosign attempt (2026-08-08): PARTIAL; the verified Harbor v2.15.2 offline package (SHA-256 `67517e0ba4a3f9db90731aa560dacbc0b24a0de04d5a1b428c7d32ea96656432`) was loaded locally, all nine Harbor services became healthy over disposable HTTPS, and the verified Cosign v3.0.6 Windows binary matched SHA-256 `9b85a88ebff2d9dd30ff4984a6f61f2cedc232dd87d81fa7f2ff3c0ed96c241c`. A private `livecosign` project and project-scoped Robot were created, but Docker Desktop's daemon proxy routed the registry push to unavailable `127.0.0.1:7897`; no signed artifact or attestation was therefore created and the live production matrix was not run. The named Compose project and network were stopped and removed; the Docker CA trust entry was removed. Live acceptance remains PARTIAL.
...
2. Configure Docker Desktop's daemon `no-proxy` list to include `harbor.localhost` (or provide an equivalent direct registry route), then continue the healthy Harbor fixture with fixed-key signature, SLSA attestation, and the gated production positive/negative matrix using `scripts/test-live-harbor-cosign.ps1`.
